...
export async function POST(request: NextRequest) {
  ...
  await supabase.from('re_update_log').insert({
    update_type: 'manual_monthly',
    description: `Added monthly data for ${body.month_label}`,
    data_snapshot: body,
    updated_by: user.id,
  });
...
export async function PUT(request: NextRequest) {
  ...
  await supabase.from('re_update_log').insert({
    update_type: 'manual_monthly',
    description: `Updated monthly data for ${updates.month_label ?? id}`,
